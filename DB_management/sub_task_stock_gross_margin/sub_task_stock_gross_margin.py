...
for current_entry in cursor:
    if ((current_entry[3] == '上櫃') or (current_entry[3] == '上市')) and (current_entry[4] != 'ETF'):
        ...
        sql_cmd = 'select net_profit_margin from stock_quarter where rowid =' + str(rowid[0])
        c_stock.execute(sql_cmd)
        net_profit_margin_tmp0 = c_stock.fetchone()[0]
        if gross_margin_tmp0 == None:
            conn_stock.close()
            continue
        n = 0
        while 1:
            if (rowid[0] - n - 1) < 1:
                break
            sql_cmd = 'select gross_margin from stock_quarter where rowid =' + str(rowid[0] - n - 1)
            c_stock.execute(sql_cmd)
            gross_margin_tmp1 = c_stock.fetchone()[0]
            sql_cmd = 'select operating_profit_margin from stock_quarter where rowid =' + str(rowid[0] - n - 1)
            c_stock.execute(sql_cmd)
            operating_profit_margin_tmp1 = c_stock.fetchone()[0]
            sql_cmd = 'select net_profit_margin from stock_quarter where rowid =' + str(rowid[0] - n - 1)
            c_stock.execute(sql_cmd)
            net_profit_margin_tmp1 = c_stock.fetchone()[0]

            if (gross_margin_tmp0 > gross_margin_tmp1) and (operating_profit_margin_tmp0 > operating_profit_margin_tmp1) and (net_profit_margin_tmp0 > net_profit_margin_tmp1):
                n = n +1
            else:
                break
        ...
        if n > 8:
            ...
            """
            result_stock.append([current_entry[0], current_entry[1], current_entry[4], date_ID_str, str(n+1), gross_margin_tmp0])
            for i in range(0,7):
                colors[i] = 0

            result_color.append([colors[0], colors[1], colors[2], colors[3], colors[4], colors[5]])
            print(n+1)
        conn_stock.close()
conn.close()
...
filename = 'stock_gross_margin.html'
